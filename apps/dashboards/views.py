...
def chat_gemini(request):
    if request.method == 'POST':
        try:
            ...
            if not user_message: return JsonResponse({'error': 'Mensagem vazia'}, status=400)

            api_key = "AIzaSyA_kD9EqUCsXV-Svi-xcC_O8qLFSI3arS8" 
            if not api_key: return JsonResponse({'response': '⚠️ Erro de configuração: API Key do Gemini não encontrada.'})

            genai.configure(api_key=api_key)
            
            # Recuperação básica de contexto (simplificada)
            user_context = f"Usuário: {request.user.get_full_name()}. "
            if hasattr(request.user, 'profile'): user_context += f"Perfil: {request.user.profile.tipo}"

            contexto_ia = f"Você é o assistente virtual do SENAI School Manager. Responda em português. Contexto: {user_context}"
